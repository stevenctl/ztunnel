...
impl Inbound {
    ...
    async fn serve_connect(
        ...
        let from_waypoint = proxy::check_from_waypoint(pi.state.clone(), &upstream, conn.src_identity.as_ref()).await;
        let from_gateway =
            proxy::check_from_network_gateway(pi.state.clone(), &upstream, conn.src_identity.as_ref()).await;
